...
通过反射机制，从类型中解析出各个参数的名字和对应的类型，并且判断参数名是否合法。 `StaticInvoke`会调用static返回正确的类型 `Invoke`方法，传入被转化为Expression的类、函数名和对应返回类型，从而有效的调用对应函数。
...
上图是对select * from table where a>100 group by b parse后的图
